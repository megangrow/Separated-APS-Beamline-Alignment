...
mcp = FastMCP("Run All Images (Step 3)")
...
def open_data():
    ...
    x_coor = int(x)
    y_coor = int(y)
    return all_image_masks, edges, answer_normalization, width, height, x_coor, y_coor

all_image_masks, edges, answer_normalization, width, height, x_coor, y_coor = open_data()
angle_rotation = 5
time_exposure = 2
pixel_size = 1.172
capture_index = 0
...
# Save parameters, coordinates + reg and scatter plot images
def save_data(x_coor, y_coor, reg, scatter, params, first_midpoint):
    ...
    reg.write_image(os.path.join(output, "reg_image.png"))
    scatter.write_image(os.path.join(output, "scatter_plot.png"))
