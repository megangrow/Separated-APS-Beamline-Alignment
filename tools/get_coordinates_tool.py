...
def get_coordinates_core():
    ...
    config.object_pt = get_object_point(masks)
    print(f"Beam point: {beam_pt}")
    print(f"Object point: {config.object_pt}")
    save_images(beam_mask, config.beam_pt, config.object_pt, config.output)
...
def save_images(beam_mask, beam_point, object_point, out):
