...
def center_pin_core():
    ...
    move_aero_x = ((config.offset - (beam_center / 2)) * config.pixel_size) / 1000
    ...
    config.mtr_samXE.move(move_sam_x, relative=True, wait=True)
    config.mtr_samYE.move(move_sam_y, relative=True, wait=True)
    config.mtr_aeroXE.move(move_aero_x, relative=True, wait=True)
